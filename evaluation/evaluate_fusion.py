...
def main():
    print("\n📊 Evaluating Fusion Model (IBNR-65 + DenseNet-64)\n")

    dataset = EuroSATDataset(DATA_DIR, transform=get_val_transforms())
    dataloader = DataLoader(dataset, BATCH_SIZE, shuffle=False)

    model = FusionNet(num_classes=10)
    weight_path = os.path.join(WEIGHTS_DIR, "fusion_lulc.pth")

    if not os.path.exists(weight_path):
        raise FileNotFoundError(f"Fusion weights not found at {weight_path}")
    ...
    report = f"""
Fusion Model Evaluation Results (EuroSAT)
...
"""
    ...
    output_file = os.path.join(OUTPUT_DIR, "fusion_metrics.txt")
