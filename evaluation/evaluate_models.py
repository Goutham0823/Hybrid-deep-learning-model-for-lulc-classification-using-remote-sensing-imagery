...
def main():
    ...
    weight_path = os.path.join(WEIGHTS_DIR, "densenet64_lulc.pth")

    if not os.path.exists(weight_path):
        raise FileNotFoundError(
            f"DenseNet-64 weights not found at {weight_path}"
        )
    ...
    report = f"""
DenseNet-64 Evaluation Results (EuroSAT)
...
"""
    ...
    output_file = os.path.join(
        OUTPUT_DIR, "densenet64_metrics.txt"
    )
