...
def main():
    print("\n📊 Evaluating IBNR-65 Model\n")

    dataset = EuroSATDataset(DATA_DIR, transform=get_val_transforms())
    dataloader = DataLoader(dataset, BATCH_SIZE, shuffle=False)

    model = IBNR65(num_classes=10)
    weight_path = os.path.join(WEIGHTS_DIR, "ibnr65_lulc.pth")

    if not os.path.exists(weight_path):
        raise FileNotFoundError(f"IBNR-65 weights not found at {weight_path}")
    ...
    report = f"""
IBNR-65 Evaluation Results (EuroSAT)
...
"""
    ...
    output_file = os.path.join(OUTPUT_DIR, "ibnr65_metrics.txt")
